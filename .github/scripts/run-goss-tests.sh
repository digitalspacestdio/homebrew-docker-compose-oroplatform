...
wait_for_http_port() {
  ...
  do
    ...
    if curl -s -f -m 5 "http://${NGINX_HOST}:${HTTP_PORT}" >/dev/null 2>&1
    then
    ...
    if timeout 5 bash -c "</dev/tcp/${NGINX_HOST}/${HTTP_PORT}" >/dev/null 2>&1
    then
...
run_goss_tests() {
  ...
  if goss -g "${GOSS_FILE}" validate --format junit >"/tmp/goss-results-${UNIQUE_PROJECT_NAME}.xml"
  then
