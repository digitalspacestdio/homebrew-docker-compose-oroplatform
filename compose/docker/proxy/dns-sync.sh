...
update_hosts() {
  ...
  then
    ...
    do
      ...
      if [[ -z "${container_ip}" ]] || [[ "${container_ip}" == "<no value>" ]]
      then
      ...
      then
        ...
        do
          h=$(echo "${h}" | xargs) # Trim
          if [[ -n "${h}" ]] && ! grep -q "^${h}$" "${temp_hosts_file}" 2>/dev/null
          then
      ...
      then
        ...
        while IFS= read -r label_line
        do
          if [[ "${label_line}" =~ traefik\.http\.routers\..*\.rule=(.+)$ ]]
          then
            ...
            do
              if [[ -n "${hostname}" ]] && ! grep -q "^${hostname}$" "${temp_hosts_file}" 2>/dev/null
              then
  ...
  if grep -q "${MARKER_START}" "${HOSTS_FILE}" 2>/dev/null
  then
