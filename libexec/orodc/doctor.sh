...
find_doctor_config_dir() {
  ...
  # Try paths in order
  local doctor_dir=""
  if [[ -d "${BREW_PREFIX}/Homebrew/Library/Taps/digitalspacestdio/homebrew-docker-compose-oroplatform/compose/docker/doctor" ]]
  then
    doctor_dir="${BREW_PREFIX}/Homebrew/Library/Taps/digitalspacestdio/homebrew-docker-compose-oroplatform/compose/docker/doctor"
  elif [[ -d "${BREW_PREFIX}/share/docker-compose-oroplatform/compose/docker/doctor" ]]
  then
    doctor_dir="${BREW_PREFIX}/share/docker-compose-oroplatform/compose/docker/doctor"
  elif [[ -d "${SCRIPT_DIR}/../../compose/docker/doctor" ]]
  then
...
detect_oro_version() {
  ...
  then
    ...
    then
      ...
      if [[ "${major}" -gt 6 ]] || [[ "${major}" -eq 6 && "${minor}" -ge 1 ]]
      then
        echo "v${major_minor}"
        return 0
      else
  ...
  case "${os_type}" in
    alpine)
      ...
      docker exec "${container_name}" sh -c "
        if ! command -v goss >/dev/null 2>&1
        then
...
get_goss_config_file() {
  ...
  then
    ...
    if [[ "${db_schema}" == "pgsql" ]] || [[ "${db_schema}" == "postgresql" ]] || [[ "${db_schema}" == "pdo_pgsql" ]] || [[ "${db_schema}" == "postgres" ]]
    then
      db_schema="postgresql"
    elif [[ "${db_schema}" == "mariadb" ]] || [[ "${db_schema}" == "pdo_mysql" ]] || [[ "${db_schema}" == "mysql" ]]
    then
      db_schema="mysql"
    fi
    ...
    if [[ "${version}" == "legacy" ]]
    then
      if [[ -n "${doctor_dir}" ]] && [[ -n "${db_schema}" ]] && [[ -f "${doctor_dir}/legacy/database-${db_schema}.yaml" ]]
      then
      ...
      if [[ -n "${doctor_dir}" ]] && [[ -n "${db_schema}" ]] && [[ -f "${doctor_dir}/${version}/database-${db_schema}.yaml" ]]
      then
        echo "${doctor_dir}/${version}/database-${db_schema}.yaml"
        return 0
      fi
      # Try default config (for 6.1+)
      if [[ -n "${doctor_dir}" ]] && [[ -n "${db_schema}" ]] && [[ -f "${doctor_dir}/default/database-${db_schema}.yaml" ]]
      then
  ...
  then
    ...
    if [[ -n "${doctor_dir}" ]] && [[ -f "${doctor_dir}/legacy/${service}.yaml" ]]
    then
  ...
  # Try version-specific config first (for 6.1+)
  if [[ -n "${doctor_dir}" ]] && [[ -f "${doctor_dir}/${version}/${service}.yaml" ]]
  then
  ...
  # Try default config (for 6.1+)
  if [[ -n "${doctor_dir}" ]] && [[ -f "${doctor_dir}/default/${service}.yaml" ]]
  then
...
add_database_connection_checks() {
  ...
  if [[ "${db_schema}" == "pdo_pgsql" ]] || [[ "${db_schema}" == "postgresql" ]] || [[ "${db_schema}" == "pgsql" ]] || [[ "${db_schema}" == "postgres" ]]
  then
    db_schema="postgres"
    db_port="${DC_ORO_DATABASE_PORT:-5432}"
  elif [[ "${db_schema}" == "pdo_mysql" ]] || [[ "${db_schema}" == "mysql" ]] || [[ "${db_schema}" == "mariadb" ]]
  then
    db_schema="mysql"
    db_port="${DC_ORO_DATABASE_PORT:-3306}"
  fi
...
prepare_goss_config() {
  ...
  if [[ -n "${source_config}" ]] && [[ -f "${source_config}" ]]
  then
    ...
    if [[ "${service}" == "database" ]] || [[ "${service}" == "redis" ]] || [[ "${service}" == "search" ]] || [[ "${service}" == "mq" ]] || [[ "${service}" == "mail" ]] || [[ "${service}" == "nginx" ]]
    then
      ...
      if ! grep -q "^command:" "${target_config}"
      then
        echo "" >>"${target_config}"
  ...
  if [[ "${service}" == "redis" ]] || [[ "${service}" == "search" ]] || [[ "${service}" == "mq" ]]
  then
...
then
  ...
  if [[ "${exit_code}" -eq 0 ]] && echo "${output}" | grep -q "connected"
  then
    echo "${output}"
    return 0
  else
...
get_container_info() {
  ...
  if [[ -n "${container_info}" ]] && command -v jq >/dev/null 2>&1
  then
    # Parse JSON output
  ...
  if [[ -f "${compose_file}" ]] && command -v yq >/dev/null 2>&1
  then
...
run_goss_check_for_service() {
  ...
  if [[ ! -f "${config_file}" ]] || [[ -z "${config_file}" ]]
  then
  ...
  if [[ "${service}" == "database" ]] && [[ -n "${db_check_script}" ]] && [[ -f "${db_check_script}" ]]
  then
  ...
  if [[ "${service}" == "redis" ]] || [[ "${service}" == "search" ]] || [[ "${service}" == "mq" ]]
  then
    if [[ -n "${service_check_script}" ]] && [[ -f "${service_check_script}" ]]
    then
  ...
  if [[ ${exit_code} -eq 0 ]] && grep -q "Failures:" "${log_file}" 2>/dev/null
  then
    exit_code=1
  fi
...
print_results_table() {
  ...
  for result_line in "${results_array[@]}"
  do
    ...
    elif [[ "${status}" == *"Exited"* ]] || [[ "${status}" == *"stopped"* ]]
    then
    ...
    elif [[ "${status_display}" == "stopped" ]] || [[ "${status_display}" == "not found" ]]
    then
    ...
    if [[ "${test_result}" == "FAIL" ]] && [[ -n "${log_file}" ]] && [[ -f "${log_file}" ]]
    then
      printf "Log:       %s\n" "${log_file}"
    fi
...
run_goss_checks_from_cli() {
  ...
  do
    ...
    local health_status=""
    if [[ "${status}" != "not found" ]] && [[ -n "${container_name}" ]]
    then
    ...
    if [[ "${normalized_status}" == *"not found"* ]] || [[ -z "${status}" ]] || [[ "${status}" == "unknown" ]]
    then
      test_result="SKIP"
    elif [[ "${normalized_status}" == *"restarting"* ]]
    then
      # Container is restarting - this is a problem state, mark as FAIL
      test_result="FAIL"
      overall_failed=1
    elif [[ -n "${health_status}" ]] && [[ "${health_status}" != "healthy" ]]
    then
      ...
      test_result="FAIL"
      overall_failed=1
    elif [[ "${normalized_status}" == *"exited"* ]] || [[ "${normalized_status}" == *"stopped"* ]]
    then
      # Container exists but is stopped - skip test
      test_result="SKIP"
    elif [[ "${normalized_status}" == *"up"* ]] || [[ "${normalized_status}" == *"running"* ]]
    then
