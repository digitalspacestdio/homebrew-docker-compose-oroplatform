...
is_interactive() {
  ...
  if [[ -n "${DC_ORO_IS_INTERACTIVE_MENU:-}" ]]
  then
    return 0
  fi
...
update_env_file() {
  ...
  then
    ...
    if [[ -z "${project_name}" ]] && [[ -n "${DC_ORO_APPDIR:-}" ]]
    then
...
manage_domains_interactive() {
  ...
  display_domain_list() {
    ...
    then
      ...
      then
        ...
        if [[ ${index} -ge 0 ]] && [[ ${index} -lt ${#domains[@]} ]]
        then
        ...
        if [[ "${remove_arg}" == "${default_domain_name}" ]] || [[ "${remove_arg}" == "${DC_ORO_NAME}" ]]
        then
      ...
      do
        domain=$(echo "${domain}" | sed 's/^[[:space:]]*//;s/[[:space:]]*$//')
        if [[ "${domain}" == "${domain_to_remove}" ]] && [[ "${found}" == "false" ]]
        then
          found=true
  ...
  then
    ...
    # Remove if empty
    if [[ -n "${TARGET_ENV_FILE}" ]] && [[ -f "${TARGET_ENV_FILE}" ]] && grep -q "^DC_ORO_EXTRA_HOSTS=" "${TARGET_ENV_FILE}" 2>/dev/null
    then
      if [[ "$(uname)" == "Darwin" ]]
      then
  ...
  case "${action}" in
    ...
    remove)
      ...
      then
        ...
        if [[ -f "${env_file}" ]] && grep -q "^DC_ORO_EXTRA_HOSTS=" "${env_file}" 2>/dev/null
        then
          if [[ "$(uname)" == "Darwin" ]]
          then
