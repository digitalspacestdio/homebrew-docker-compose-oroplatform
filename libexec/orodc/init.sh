...
then
  ...
  if [[ -n "${DEBUG:-}" ]] && [[ -n "${DETECTED_CMS_TYPE}" ]]
  then
...
if [[ "${DETECTED_CMS_TYPE_DISPLAY}" == "base" ]]
then
...
if prompt_yes_no "Set CMS type explicitly?" "$([[ -n "${EXISTING_CMS_TYPE}" ]] && echo yes || echo no)"
then
  CMS_TYPES=("php-generic" "magento" "oro")
  DEFAULT_CMS=""

  # Determine default: existing > auto-detected > php-generic
  if [[ -n "${EXISTING_CMS_TYPE}" ]]
  then
    DEFAULT_CMS="${EXISTING_CMS_TYPE}"
  elif [[ -n "${AUTO_DETECTED_CMS}" ]]
  then
...
init_page_php() {
  ...
  if [[ -n "${SELECTED_PHP_IMAGE}" ]] && [[ ! "${SELECTED_PHP_IMAGE}" =~ ^ghcr\.io/digitalspacestdio/orodc-php-node-symfony: ]]
  then
    USE_CUSTOM_PHP=true
  fi

  if prompt_yes_no "Use custom PHP image?" "$([[ "${USE_CUSTOM_PHP}" = true ]] && echo yes || echo no)"
  then
    ...
    if [[ -z "${SELECTED_PHP_IMAGE}" ]] && [[ -n "${EXISTING_PHP_IMAGE}" ]]
    then
    ...
    if [[ -n "${EXISTING_NODE_VERSION}" ]] && [[ " ${COMPATIBLE_NODE_VERSIONS[*]} " == *" ${EXISTING_NODE_VERSION} "* ]]
    then
      ...
      if [[ ! " ${COMPATIBLE_NODE_VERSIONS[*]} " == *" ${DEFAULT_NODE} "* ]]
      then
    ...
    if [[ "${SELECTED_PHP}" == "7.1" ]] || [[ "${SELECTED_PHP}" == "7.2" ]] || [[ "${SELECTED_PHP}" == "7.3" ]]
    then
...
# Determine if using custom image
USE_CUSTOM_PHP=false
if [[ -n "${EXISTING_PHP_IMAGE}" ]] && [[ ! "${EXISTING_PHP_IMAGE}" =~ ^ghcr\.io/digitalspacestdio/orodc-php-node-symfony: ]]
then
  USE_CUSTOM_PHP=true
fi
...
then
  ...
  then
    ...
    if [[ -n "${EXISTING_COMPOSER_VERSION}" ]]
    then
...
if prompt_yes_no "Use custom database image?" "$([[ "${USE_CUSTOM_DB}" = true ]] && echo yes || echo no)"
then
  ...
  # If empty, keep existing
  if [[ -z "${SELECTED_DB_IMAGE}" ]] && [[ -n "${EXISTING_DB_IMAGE}" ]]
  then
  ...
  if [[ "${EXISTING_DB_SCHEMA}" == "mysql" ]]
  then
    DEFAULT_DB_TYPE="MySQL"
  else
    # Use detected CMS type (determined at the beginning) to determine default database
    if [[ -n "${DETECTED_CMS_TYPE}" ]]
    then
      # Set MySQL as default for: marello, magento, base (php-generic), wintercms
      if [[ "${DETECTED_CMS_TYPE}" == "marello" ]] || [[ "${DETECTED_CMS_TYPE}" == "magento" ]] || [[ "${DETECTED_CMS_TYPE}" == "base" ]] || [[ "${DETECTED_CMS_TYPE}" == "wintercms" ]]
      then
        DEFAULT_DB_TYPE="MySQL"
        if [[ -n "${DEBUG:-}" ]]
        then
  ...
  then
    ...
    if [[ "${EXISTING_DB_SCHEMA}" == "mysql" ]] && [[ " ${MYSQL_VERSIONS[*]} " == *" ${EXISTING_DB_VERSION} "* ]]
    then
...
if [[ -n "${EXISTING_SEARCH_IMAGE}" ]] && [[ ! "${EXISTING_SEARCH_IMAGE}" =~ ^(docker\.elastic\.co/elasticsearch/elasticsearch:|opensearchproject/opensearch:) ]]
then
  USE_CUSTOM_SEARCH=true
fi

if prompt_yes_no "Use custom search engine image?" "$([[ "${USE_CUSTOM_SEARCH}" = true ]] && echo yes || echo no)"
then
  ...
  # If empty, keep existing
  if [[ -z "${SELECTED_SEARCH_IMAGE}" ]] && [[ -n "${EXISTING_SEARCH_IMAGE}" ]]
  then
  ...
  if [[ -n "${EXISTING_SEARCH_ENGINE}" ]]
  then
    DEFAULT_SEARCH_TYPE="${EXISTING_SEARCH_ENGINE}"
  elif [[ "${DETECTED_CMS_TYPE}" == "magento" ]]
  then
  ...
  # Select version based on type (sorted newest to oldest)
  if [[ "${SELECTED_SEARCH_TYPE}" == "Elasticsearch" ]]
  then
    ...
    if [[ "${EXISTING_SEARCH_ENGINE}" == "Elasticsearch" ]] && [[ " ${ELASTIC_VERSIONS[*]} " == *" ${EXISTING_SEARCH_VERSION} "* ]]
    then
    ...
    if [[ "${EXISTING_SEARCH_ENGINE}" == "OpenSearch" ]] && [[ " ${OPENSEARCH_VERSIONS[*]} " == *" ${EXISTING_SEARCH_VERSION} "* ]]
    then
...
if [[ -n "${EXISTING_CACHE_IMAGE}" ]] && [[ ! "${EXISTING_CACHE_IMAGE}" =~ ^(redis:|eqalpha/keydb:|valkey/valkey:) ]]
then
  USE_CUSTOM_CACHE=true
fi

if prompt_yes_no "Use custom cache image?" "$([[ "${USE_CUSTOM_CACHE}" = true ]] && echo yes || echo no)"
then
  ...
  # If empty, keep existing
  if [[ -z "${SELECTED_CACHE_IMAGE}" ]] && [[ -n "${EXISTING_CACHE_IMAGE}" ]]
  then
  ...
  elif [[ "${SELECTED_CACHE_TYPE}" == "Valkey" ]]
  then
    VALKEY_VERSIONS=("9.0" "8.0" "7.2")
    # Only use existing version if it's valid for Valkey and type hasn't changed
    if [[ "${EXISTING_CACHE_ENGINE}" == "Valkey" ]] && [[ " ${VALKEY_VERSIONS[*]} " == *" ${EXISTING_CACHE_VERSION} "* ]]
    then
    ...
    if [[ "${EXISTING_CACHE_ENGINE}" == "KeyDB" ]] && [[ " ${KEYDB_VERSIONS[*]} " == *" ${EXISTING_CACHE_VERSION} "* ]]
    then
...
if [[ -n "${EXISTING_RABBITMQ_IMAGE}" ]] && [[ ! "${EXISTING_RABBITMQ_IMAGE}" =~ ^rabbitmq: ]]
then
  USE_CUSTOM_RABBITMQ=true
fi

if prompt_yes_no "Use custom RabbitMQ image?" "$([[ "${USE_CUSTOM_RABBITMQ}" = true ]] && echo yes || echo no)"
then
  ...
  # If empty, keep existing
  if [[ -z "${SELECTED_RABBITMQ_IMAGE}" ]] && [[ -n "${EXISTING_RABBITMQ_IMAGE}" ]]
  then
...
if prompt_yes_no "Save configuration to project directory (${LOCAL_ENV_DISPLAY})?" "no"
then
...
EOF
  ...
  if ! grep -q "# Last updated:" "${TARGET_ENV_FILE}" 2>/dev/null
  then
    if [[ "$(uname)" == "Darwin" ]]
    then
      sed -i '' "1i\\
