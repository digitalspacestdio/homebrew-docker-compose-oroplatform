...
if [[ "${DC_ORO_PURGE_FORCE:-}" == "1" ]] || [[ "${DC_ORO_PURGE_FORCE:-}" == "true" ]]
then
  SKIP_CONFIRMATION=true
fi
...
then
  if ! confirm_yes_no "Are you sure you want to purge everything?"
  then
...
then
  ...
  if [[ -n "${remaining_networks}" ]]
  then
    echo "${remaining_networks}" | while read -r network_name; do
      # Filter out default networks (bridge, host, none) and shared networks
      if [[ -n "${network_name}" ]] && [[ "${network_name}" != "bridge" ]] && [[ "${network_name}" != "host" ]] && [[ "${network_name}" != "none" ]] && [[ "${network_name}" != "dc_shared_net" ]]
      then
  ...
  if [[ -n "${all_project_images}" ]]
  then
    echo "${all_project_images}" | while read -r image_name; do
      # Skip base images and external images (ghcr.io, docker.elastic.co, etc.)
      if [[ -n "${image_name}" ]] &&
         [[ ! "${image_name}" =~ ^(ghcr\.io|docker\.elastic\.co|opensearchproject|valkey|redis|mysql|rabbitmq|percona|xhgui|oroinc|busybox) ]]
      then
        docker rmi -f "${image_name}" 2>/dev/null || true
      fi
    done
...
if [[ -n "${DC_ORO_CONFIG_DIR:-}" ]] && [[ -d "${DC_ORO_CONFIG_DIR}" ]]
then
...
if [[ "${SKIP_CONFIRMATION}" == "true" ]] || [[ "${PURGE_CONFIRMED:-}" == "true" ]]
then
  for dir in "${config_dirs_to_remove[@]}"
  do
    if [[ -d "${dir}" ]]
    then
