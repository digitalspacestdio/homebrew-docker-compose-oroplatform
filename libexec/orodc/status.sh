...
then
  ...
  elif [[ -f "composer.json" ]] || [[ -f ".env.orodc" ]]
  then
...
check_initialized() {
  ...
  if [[ -f "${local_config}" ]] || [[ -f "${global_config}" ]]
  then
    initialized=true
  fi
...
main() {
  ...
  if [[ -n "${project_name}" ]] && [[ "${project_name}" != "default" ]]
  then
  ...
  if [[ -n "${project_name}" ]] && [[ "${project_name}" != "default" ]] && [[ -n "${env_status_display}" ]]
  then
  ...
  if [[ "${project_exists}" == "true" ]] && [[ "${cms_type}" != "unknown" ]]
  then
    project_ready=true
  fi
  ...
  if [[ "${project_ready}" == "true" ]]
  then
    msg_key_value "Status" "Ready to work"
  elif [[ "${initialized}" == "true" ]] && [[ "${cms_type}" != "unknown" ]] && [[ "${project_exists}" == "true" ]]
  then
    msg_key_value "Status" "Ready to work"
  elif [[ "${directory_empty}" == "true" ]] && [[ "${initialized}" == "true" ]]
  then
    msg_key_value "Status" "Environment ready, waiting for project code"
  elif [[ "${directory_empty}" == "true" ]]
  then
