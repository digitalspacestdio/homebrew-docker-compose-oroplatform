...
export_database_interactive() {
  ...
  if [[ -z "${db_port}" ]]
  then
    if [[ "${DC_ORO_DATABASE_SCHEMA}" == "mariadb" ]] || [[ "${DC_ORO_DATABASE_SCHEMA}" == "mysql" ]] || [[ "${DC_ORO_DATABASE_SCHEMA}" == "pdo_mysql" ]]
    then
      db_port="3306"
    else
  ...
  if [[ ${DC_ORO_DATABASE_SCHEMA} == "pgsql" ]] || [[ ${DC_ORO_DATABASE_SCHEMA} == "postgres" ]] || [[ ${DC_ORO_DATABASE_SCHEMA} == "postgresql" ]] || [[ ${DC_ORO_DATABASE_SCHEMA} == "pdo_pgsql" ]]
  then
  ...
  elif [[ "${DC_ORO_DATABASE_SCHEMA}" == "mariadb" ]] || [[ "${DC_ORO_DATABASE_SCHEMA}" == "mysql" ]] || [[ "${DC_ORO_DATABASE_SCHEMA}" == "pdo_mysql" ]]
  then
  ...
  if ! run_with_spinner "Exporting database" "${export_cmd}"
  then
  ...
  if [[ ! -f "${DB_DUMP}" ]] || [[ $(stat -f%z "${DB_DUMP}" 2>/dev/null || stat -c%s "${DB_DUMP}" 2>/dev/null || echo 0) -lt 1024 ]]
  then
