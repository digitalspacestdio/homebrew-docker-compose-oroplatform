...
do
  ...
  if [[ "${arg}" == "--from-domain" ]] && [[ -n "${next_arg}" ]]
  then
    FROM_DOMAIN="${next_arg}"
    i=$((i + 2))
  elif [[ "${arg}" == "--to-domain" ]] && [[ -n "${next_arg}" ]]
  then
    TO_DOMAIN="${next_arg}"
    i=$((i + 2))
  elif [[ "${arg}" == --from-domain=* ]]
  then
...
validate_domain() {
  ...
  if [[ ! "${domain}" =~ \. ]] && [[ ! "${domain}" =~ \.local$ ]] && [[ ! "${domain}" =~ ^localhost$ ]]
  then
    return 1
  fi
...
prompt_domain_replacement() {
  ...
  if [[ -n "${FROM_DOMAIN}" ]] && [[ -n "${TO_DOMAIN}" ]]
  then
    # Validate domains provided via flags
    if ! validate_domain "${FROM_DOMAIN}"
    then
      ...
      TO_DOMAIN=""
      return 1
    elif ! validate_domain "${TO_DOMAIN}"
    then
  ...
  echo "" >&2
  if confirm_yes_no "Replace domain names in database dump?"
  then
    ...
    do
      ...
      if [[ -z "${FROM_DOMAIN}" ]] && [[ -n "${previous_from}" ]]
      then
      ...
      then
        ...
        TO_DOMAIN=""
        break
      elif validate_domain "${FROM_DOMAIN}"
      then
        break
      else
...
save_domain_replacement() {
  ...
  then
    ...
    if grep -q "^LAST_FROM_DOMAIN=" "${global_config_file}" 2>/dev/null
    then
      if [[ "$(uname)" == "Darwin" ]]
      then
    ...
    if grep -q "^LAST_TO_DOMAIN=" "${global_config_file}" 2>/dev/null
    then
      if [[ "$(uname)" == "Darwin" ]]
      then
...
perform_database_import() {
  ...
  if [[ "${DC_ORO_DATABASE_SCHEMA}" == "mysql" ]] && [[ "${DOCKER_COMPOSE_BIN_CMD}" != *"run-only"* ]]
  then
  ...
  if [[ ${DC_ORO_DATABASE_SCHEMA} == "pdo_pgsql" ]] || [[ ${DC_ORO_DATABASE_SCHEMA} == "postgres" ]]
  then
  ...
  PV_AVAILABLE=false
  if ${db_cli_cmd} run --rm --no-deps -T database-cli bash -c "command -v pv >/dev/null 2>&1" >/dev/null 2>&1
  then
    PV_AVAILABLE=true
  elif [[ -n "${DEBUG:-}" ]]
  then
  ...
  if echo "${DB_DUMP_BASENAME}" | grep -i 'sql\.gz$' >/dev/null
  then
...
import_database_interactive() {
  ...
  then
    ...
    if [[ "${input}" =~ ^[0-9]+$ ]] && [[ ${input} -ge 1 ]] && [[ ${input} -le ${#dumps[@]} ]]
    then
  ...
  if [[ -z "${selected_file}" ]] || [[ ! -r "${selected_file}" ]]
  then
