...
# PostgreSQL volumes
if docker volume ls --format '{{.Name}}' 2>/dev/null | grep -q "^${DC_ORO_NAME:-}_database" 2>/dev/null
then
  docker volume rm "${DC_ORO_NAME:-}_database" 2>/dev/null || true
fi
# MySQL volumes
if docker volume ls --format '{{.Name}}' 2>/dev/null | grep -q "^${DC_ORO_NAME:-}_mysql" 2>/dev/null
then
