...
run_command_with_menu_return() {
  ...
  if ! command -v "${orodc_cmd}" >/dev/null 2>&1
  then
...
show_interactive_menu() {
  ...
  elif [[ "${current_status}" == "uninitialized" ]]
  then
  ...
  display_menu_with_selection() {
    ...
    then
      ...
      get_group_item environment 0 "${sel}"
      get_group_item maintenance 0 "${sel}"
      echo "" >&2
      get_group_item environment 1 "${sel}"
      get_group_item maintenance 1 "${sel}"
      echo "" >&2
      get_group_item environment 2 "${sel}"
      get_group_item maintenance 2 "${sel}"
      echo "" >&2
      get_group_item environment 3 "${sel}"
      printf "  %-32s" "" >&2
      echo "" >&2
      get_group_item environment 4 "${sel}"
      printf "  %-32s" "" >&2
      echo "" >&2
      get_group_item environment 5 "${sel}"
      printf "  %-32s" "" >&2
      echo "" >&2
      ...
      get_group_item database 0 "${sel}"
      get_group_item configuration 0 "${sel}"
      echo "" >&2
      get_group_item database 1 "${sel}"
      get_group_item configuration 1 "${sel}"
      echo "" >&2
      get_group_item database 2 "${sel}"
      get_group_item configuration 2 "${sel}"
      echo "" >&2
      ...
      get_group_item oro 0 "${sel}"
      get_group_item proxy 0 "${sel}"
      echo "" >&2
      get_group_item oro 1 "${sel}"
      get_group_item proxy 1 "${sel}"
      echo "" >&2
      get_group_item oro 2 "${sel}"
      get_group_item proxy 2 "${sel}"
      echo "" >&2
      get_group_item oro 3 "${sel}"
      printf "  %-32s" "" >&2
      echo "" >&2
      get_group_item oro 4 "${sel}"
      printf "  %-32s" "" >&2
      echo "" >&2
  ...
  do
    ...
    if ! IFS= read -rsn1 key </dev/tty 2>/dev/null
    then
      # Fallback to stdin if /dev/tty is not available
      if ! IFS= read -rsn1 key 2>/dev/null
      then
    ...
    then
      ...
      local rest=""
      if IFS= read -rsn2 -t 1 rest </dev/tty 2>/dev/null || IFS= read -rsn2 -t 1 rest 2>/dev/null
      then
    ...
    then
      ...
      do
        ...
        if ! IFS= read -rsn1 -t 0.5 additional_digit </dev/tty 2>/dev/null
        then
          # If /dev/tty read failed, try stdin
          if ! IFS= read -rsn1 -t 0.5 additional_digit 2>/dev/null
          then
        ...
        if [[ -n "${additional_digit}" ]] && [[ "${additional_digit}" =~ ^[0-9]$ ]]
        then
          # Add digit to buffer
      ...
      if [[ "${num_input}" =~ ^[1-9]$ ]] || [[ "${num_input}" =~ ^1[0-9]$ ]] || [[ "${num_input}" =~ ^2[0-7]$ ]]
      then
  ...
  case "${choice}" in
    1)
      ...
      then
        ...
        if [[ -z "${DC_ORO_NAME:-}" ]] || [[ -z "${DC_ORO_APPDIR:-}" ]]
        then
