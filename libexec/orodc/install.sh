...
run_post_install_scripts() {
  ...
  then
    if jq -e '.scripts."post-install-cmd"?' "${composer_file}" >/dev/null 2>&1
    then
      has_post_install=true
    fi
  else
    if grep -q '"post-install-cmd"' "${composer_file}" 2>/dev/null
    then
      has_post_install=true
    fi
  fi

  if [[ "${has_post_install}" == "false" ]]
  then
  ...
  if ! run_with_spinner "Executing post-install scripts" "${script_cmd}"
  then
...
if [[ "${DC_ORO_DATABASE_HOST:-database}" == "localhost" ]] || [[ "${DC_ORO_DATABASE_HOST:-database}" == "127.0.0.1" ]]
then
