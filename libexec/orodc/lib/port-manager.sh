...
find_and_export_ports() {
  ...
  then
    ...
    then
      ...
      then
        ...
        if [[ -n "${script_file}" ]] && [[ -f "${script_file}" ]]
        then
  ...
  if [[ -z "${find_port_bin}" ]] || [[ ! -x "${find_port_bin}" ]]
  then
  ...
  if [[ ${exit_code} -ne 0 ]] || [[ -z "${BATCH_PORTS}" ]]
  then
  ...
  do
    ...
    case "${service}" in
      ...
      xhgui)
        ...
        ;;
      database)
        if [[ -z "${DC_ORO_PORT_MYSQL}" ]] || [[ "${DC_ORO_PORT_MYSQL}" == "${DC_ORO_PORT_PREFIX}06" ]]
        then
