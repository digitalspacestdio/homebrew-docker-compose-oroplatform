...
resolve_bin() {
  ...
  then
    ...
    if [[ -n "${DEBUG}" ]]; then echo "DEBUG: Found ${bin_name} in PATH: ${found_path}" >&2; fi
    echo "${found_path}"
    return 0
  fi
...
parse_dsn_uri() {
  ...
  local clean_uri=""
  if [[ "${schema}" == "sqlite" && "${dbname}" == ":memory:" ]]
  then
    clean_uri="sqlite::memory:"
  elif [[ "${schema}" == "sqlite" ]]
  then
    clean_uri="sqlite://${dbname}"
  elif [[ "${schema}" == "dbal" && -z "${rest}" ]]
  then
    clean_uri="${schema}:"
  else
    clean_uri="${schema}://"
...
is_oro_project() {
  ...
  then
    ...
    if grep -qE '"(oro/platform|oro/commerce|oro/crm|oro/customer-portal|marello/marello|marellocommerce/marello)"' "${composer_file}" 2>/dev/null
    then
      return 0
    fi
  fi
...
detect_cms_type() {
  ...
  then
    ...
    then
      ...
      if grep -qE '"(magento/product-community-edition|magento/product-enterprise-edition|magento/magento-cloud-metapackage|mage-os/mage-os)"' "${composer_file}" 2>/dev/null
      then
        echo "magento"
        return 0
      fi

      if grep -qE '"(symfony/symfony|symfony/framework-bundle|symfony/flex)"' "${composer_file}" 2>/dev/null
      then
        echo "symfony"
        return 0
      fi
      ...
      # Check for WinterCMS (before Laravel, as WinterCMS is Laravel-based)
      # Note: WinterCMS is the community fork of OctoberCMS, they are compatible
      if grep -qE '"(wintercms/winter|october/october)"' "${composer_file}" 2>/dev/null
      then
        echo "wintercms"
        return 0
      fi

      if grep -qE '"laravel/framework"' "${composer_file}" 2>/dev/null
      then
        echo "laravel"
        return 0
      fi
  ...
  if [[ -f "${project_dir}/bin/magento" ]] ||
     ...
     ([[ -f "${project_dir}/pub/index.php" ]] && grep -q "Magento" "${project_dir}/pub/index.php" 2>/dev/null)
  then
    echo "magento"
    return 0
  fi
  ...
  if [[ -f "${project_dir}/bin/console" ]] && ! is_oro_project
  then
    echo "symfony"
    return 0
  fi
...
is_marello_project() {
  ...
  then
    ...
    if [[ -n "${marello_packages}" ]]
    then
      return 0
    fi
  else
    # Fallback: grep-based detection
    if grep -qE '"(marello/marello|marellocommerce/marello)"' "${composer_file}" 2>/dev/null
    then
      return 0
    fi
  fi
...
detect_oro_type() {
  ...
  then
    ...
    if echo "${oro_packages}" | grep -qxE 'oro/commerce'
    then
      echo "commerce"
      return 0
    elif echo "${oro_packages}" | grep -qxE 'oro/crm'
    then
      echo "crm"
      return 0
    elif echo "${oro_packages}" | grep -qxE '(oro/platform|oro/customer-portal|marello/marello|marellocommerce/marello)'
    then
      echo "platform"
      return 0
    fi
    ...
    # Fallback: grep-based detection
    if grep -qE '"oro/commerce"' "${composer_file}" 2>/dev/null
    then
      echo "commerce"
      return 0
    elif grep -qE '"oro/crm"' "${composer_file}" 2>/dev/null
    then
      echo "crm"
      return 0
    elif grep -qE '"(oro/platform|oro/customer-portal|marello/marello|marellocommerce/marello)"' "${composer_file}" 2>/dev/null
    then
      echo "platform"
      return 0
    fi
