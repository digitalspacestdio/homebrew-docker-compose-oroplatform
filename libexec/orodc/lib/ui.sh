...
run_with_spinner() {
  ...
  if [[ -n "${DEBUG:-}" ]] || [[ -n "${VERBOSE:-}" ]] || [[ "${cmd}" =~ (--verbose|-vv) ]]
  then
...
prompt_selector() {
  ...
  do
    ...
    if [[ "${choice}" =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ ]] || [[ "${choice}" =~ ^[a-zA-Z0-9\.\-\_]+$ ]]
    then
      echo "${choice}"
      return 0
    fi
...
prompt_port() {
  ...
  do
    ...
    if [[ "${choice}" =~ ^[0-9]+$ ]] && [[ ${choice} -ge 1 ]] && [[ ${choice} -le 65535 ]]
    then
      echo "${choice}"
      return 0
    else
...
read_numeric_choice() {
  ...
  if [[ -z "${choice}" ]] && [[ -n "${default}" ]]
  then
    choice="${default}"
  fi

  # Validate pattern if provided
  if [[ -n "${choice}" ]] && [[ "${choice}" =~ ${pattern} ]]
  then
    echo "${choice}"
    return 0
  elif [[ -z "${choice}" ]]
