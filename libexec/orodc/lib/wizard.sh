...
wizard_redraw() {
  ...
  if [[ ${WIZARD_CURRENT_PAGE} -ge 0 ]] && [[ ${WIZARD_CURRENT_PAGE} -lt ${WIZARD_TOTAL_PAGES} ]]
  then
    local page_func="${WIZARD_PAGES[${WIZARD_CURRENT_PAGE}]}"
    if [[ -n "${page_func}" ]] && type "${page_func}" >/dev/null 2>&1
    then
      "${page_func}"
    fi
  fi
...
wizard_page() {
  ...
  local found=false
  for existing_page in "${WIZARD_PAGES[@]}"
  do
    if [[ "${existing_page}" == "${page_func}" ]]
    then
      found=true
      break
    fi
...
wizard_run() {
  ...
  do
    # Check bounds
    if [[ ${WIZARD_CURRENT_PAGE} -lt 0 ]]
    then
      WIZARD_CURRENT_PAGE=0
    fi
    if [[ ${WIZARD_CURRENT_PAGE} -ge ${WIZARD_TOTAL_PAGES} ]]
    then
...
wizard_simple() {
    ...
    then
      ...
      case "${input}" in
        ...
        *)
          ...
          if [[ -n "${display_func}" ]] && type "${display_func}" >/dev/null 2>&1
          then
            "${display_func}" "${input}"
            local result=$?
  ...
  read_domain_input() {
    ...
    while IFS= read -rsn1 char </dev/tty 2>/dev/null || IFS= read -rsn1 char
    do
      ...
      if [[ ${#input} -eq 0 ]]
      then
        if [[ "${char}" == "d" ]] || [[ "${char}" == "D" ]] || [[ "${char}" == "r" ]] || [[ "${char}" == "R" ]] || [[ "${char}" == "q" ]] || [[ "${char}" == "Q" ]]
        then
          is_command=true
        fi
      elif [[ "${input}" =~ ^(done|remove|q|quit|exit|refresh|r)[[:space:]]*$ ]] || [[ "${input}" =~ ^remove[[:space:]]+ ]]
      then
        is_command=true
      fi
