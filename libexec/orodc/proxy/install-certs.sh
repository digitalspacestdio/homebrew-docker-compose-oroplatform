...
if ! ${DOCKER_BIN} ps --filter "name=proxy" --format "{{.Names}}" | grep -q "^proxy$"
then
...
for i in {1..30}
do
  if ${DOCKER_BIN} ps --filter "name=proxy" --filter "health=healthy" --format "{{.Names}}" | grep -q "^proxy$"
  then
    break
  fi
  sleep 1
...
if [[ ! -f "${CERT_OUTPUT_PATH}" ]] || [[ ! -s "${CERT_OUTPUT_PATH}" ]]
then
...
then
  ...
  if sudo security add-trusted-cert -d -r trustRoot -k /Library/Keychains/System.keychain "${CERT_OUTPUT_PATH}" 2>/dev/null
  then
...
then
  ...
  then
    ...
    if [[ ! -f "${HOME}/.pki/nssdb/cert9.db" ]]
    then
      certutil -d sql:"${HOME}/.pki/nssdb" -N --empty-password 2>/dev/null
    fi

    if certutil -d sql:"${HOME}/.pki/nssdb" -A -t "C,," -n "OroDC Docker Local CA" -i "${CERT_OUTPUT_PATH}" 2>/dev/null
    then
      msg_ok "Certificate installed to NSS database"
      rm -f "${CERT_OUTPUT_PATH}"
    fi
