...
# Detect PHP version
DC_ORO_PHP_VERSION="${DC_ORO_PHP_VERSION:-8.5}"
DC_ORO_NODE_VERSION="${DC_ORO_NODE_VERSION:-24}"
...
if [[ "${DC_ORO_PHP_VERSION}" == "7.1" ]] && [[ "${DC_ORO_COMPOSER_VERSION}" == "2" ]]
then
...
if [[ "${EFFECTIVE_COMPOSER_VERSION}" != "${DC_ORO_COMPOSER_VERSION}" ]]
then
...
if [[ -f ".env.orodc" ]] && [[ -n "${DC_ORO_CONFIG_DIR:-}" ]] && [[ -d "${DC_ORO_CONFIG_DIR:-}" ]]
then
  ...
  if [[ -n "${DOCKER_COMPOSE_BIN_CMD:-}" ]] && [[ -n "${DC_ORO_NAME:-}" ]]
  then
    # Stop the stack
    msg_info "Stopping project stack..."
    down_cmd="${DOCKER_COMPOSE_BIN_CMD} down --remove-orphans"
    if run_with_spinner "Stopping containers" "${down_cmd}"
    then
...
then
  ...
  for dockerfile in "${DOCKER_DIR}"/php/Dockerfile.*."${DC_ORO_PHP_DIST}"
  do
    if [[ -f "${dockerfile}" ]]
    then
...
if [[ ! -f "${PHP_NODE_DOCKERFILE}" ]]
then
  msg_error "PHP+Node.js Dockerfile not found: ${PHP_NODE_DOCKERFILE}"
  msg_info ""
  msg_info "Available PHP+Node.js versions:"
  for dockerfile_dir in "${DOCKER_DIR}"/php-node-symfony/*/
  do
    if [[ -d "${dockerfile_dir}" ]] && [[ -f "${dockerfile_dir}Dockerfile" ]]
    then
...
if ${DOCKER_BIN} images -q "${PHP_BASE_TAG}" 2>/dev/null | grep -q .
then
...
then
  ...
  if ${DOCKER_BIN} pull "${PHP_BASE_TAG}" 2>/dev/null
  then
...
then
  ...
  if run_with_spinner "Building PHP base image" "${build_cmd}"
  then
    BUILD_END=$(date +%s)
...
# Check if image exists
if ${DOCKER_BIN} images -q "${PHP_FINAL_TAG}" 2>/dev/null | grep -q .
then
...
if [[ "${SHOULD_PULL_FINAL}" == "true" ]]
then
  msg_info "Attempting to pull from registry..."
  if ${DOCKER_BIN} pull "${PHP_FINAL_TAG}" 2>/dev/null
  then
...
then
  ...
  if run_with_spinner "Building PHP+Node.js final image" "${build_cmd}"
  then
    BUILD_END=$(date +%s)
    BUILD_DURATION=$((BUILD_END - BUILD_START))
...
then
  ...
  if [[ "${SHOULD_BUILD:-false}" == "true" ]] || [[ "${SHOULD_PULL:-false}" == "true" ]] || [[ "${SHOULD_BUILD_FINAL:-false}" == "true" ]] || [[ "${SHOULD_PULL_FINAL:-false}" == "true" ]]
  then
  ...
  if [[ "${SHOULD_REBUILD_PROJECT}" == "true" ]]
  then
    msg_info ""
    ...
    then
      ...
      if ${DOCKER_COMPOSE_BIN_CMD} config --services 2>/dev/null | grep -q "^websocket$"
      then
        services_to_build="${services_to_build} websocket"
      fi
      if ${DOCKER_COMPOSE_BIN_CMD} config --services 2>/dev/null | grep -q "^consumer$"
      then
        services_to_build="${services_to_build} consumer"
      fi

      build_cmd="${DOCKER_COMPOSE_BIN_CMD} build ${NO_CACHE_FLAG} ${services_to_build}"
      if run_with_spinner "Building project images" "${build_cmd}"
      then
        BUILD_END=$(date +%s)
        BUILD_DURATION=$((BUILD_END - BUILD_START))
...
then
  ...
  if check_in_project 2>/dev/null
  then
    ...
    if confirm_yes_no "Stop project and remove project images now?" "no"
    then
      if [[ -n "${DOCKER_COMPOSE_BIN_CMD:-}" ]]
      then
        down_cmd="${DOCKER_COMPOSE_BIN_CMD} down --remove-orphans"
...
if [[ -f ".env.orodc" ]] && [[ -n "${DC_ORO_CONFIG_DIR:-}" ]]
then
  msg_info "  - Project images (fpm, cli, consumer, websocket, ssh)"
fi
if [[ "${PROJECT_IMAGES_REMOVED}" == "true" ]]
then
