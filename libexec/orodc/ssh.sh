...
then
  ...
  if [[ -z "${ORO_SSH_PUBLIC_KEY:-}" ]] && [[ -f "${DC_ORO_CONFIG_DIR}/ssh_id_ed25519.pub" ]]
  then
  ...
  then
    ...
    if ! run_with_spinner "Starting SSH container" "${start_cmd}"
    then
      start_result=1
    else
      start_result=0
  ...
  do
    ...
    if bash -c "echo > /dev/tcp/127.0.0.1/${DC_ORO_PORT_SSH}" 2>/dev/null
    then
      break
    fi

    # Also check container health status
    if [[ $((attempt % 5)) -eq 0 ]] && [[ ${attempt} -gt 0 ]]
    then
  ...
  then
    ...
    then
      ...
      then
        ...
        if run_with_spinner "Restarting SSH container to apply new key" "${restart_cmd}"
        then
          restart_result=0
        else
          restart_result=1
...
# - ForwardAgent: host ssh-agent forwarding (default yes; DC_ORO_SSH_FORWARD_AGENT=no to disable)
...
SSH_FORWARD_AGENT=$(orodc_ssh_forward_agent)
exec ssh \
  -o SendEnv=COMPOSER_AUTH \
  -o UserKnownHostsFile=/dev/null \
  -o StrictHostKeyChecking=no \
  -o "ForwardAgent ${SSH_FORWARD_AGENT}" \
