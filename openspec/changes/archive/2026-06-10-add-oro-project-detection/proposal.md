...
- **BREAKING**: Move `consumer` service from base `docker-compose.yml` to Oro-specific `docker-compose-oro.yml`
- Include `docker-compose-oro.yml` only when Oro project detected
...
- Affected code: `bin/orodc` (detection function, menu display, compose file assembly), `compose/docker-compose.yml`, `compose/docker-compose-oro.yml`
